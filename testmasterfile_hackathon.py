...
def create_semester_gpa(gpa):
    if gpa>2.5:
        ...
        sem_sch=0
        while sem_sch<20:
            course_code=input('enter the coursecode of the class you want to take next semester to stop enter -1:')
            while checks(course_code,courses_dict,mypassedcourses):
        ...
        sem_sch=0
        while sem_sch<20:
            course_code=input('enter the coursecode of the class you are take next semester to stop enter -1:')
            while checks(course_code,courses_dict,mypassedcourses):
                list_coursecode.append(course_code)
                sem_sch=0
                for ccode in list_coursecode:
...
#this checks if you have taken the prerequisites of the courses you plan to take

##def prerequisite(mydict):
##    prerequi=[]
##    values_dict=mydict.values()
##    r=[]
##    l=[]
##    for c in values_dict:
##        r.append(c[2])
##    l=[j for i in r for j in i]
##    return l


#change to prompt user to re enter in case he doesnt have prerequisite 

##checkprerequisite(mynextcourses,courses_dict)
##for course in mynextcourses:                       
##    if checkprerequisite(mynextcourses,courses_dict):
##        print("you can take course",mynextcourses)
##    else:
##        print("you can't take course",mynextcourses,"you haven't taken the prerequisite")
######################################################################################################
#this checks if you have reached the minimum standing to take the course
